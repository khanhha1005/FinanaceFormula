...
GENERATION_METHODS = {
    ...
    2 : "Sinh công thức theo kiểu sinh f"
}
...
class Formula_generator(Base):
    ...
    def __fill_1(self, formula, struct, idx, temp_0, temp_op, temp_1, mode, add_sub_done, mul_div_done, sub_mode, check_op):
        ...
        return False
    def save_historyex(self,F_new_formula,current_formula1,current_formula2,current_operator,F_thu,F2):
        state = {
        ...
        if self.multiple_cycles:
            for w_i in range(weights.shape[0]):
                for c_i in range(self.number_cycle):
                    ...
                    if check:
                        if '2' not in str(self.generation_method)[:60]:
